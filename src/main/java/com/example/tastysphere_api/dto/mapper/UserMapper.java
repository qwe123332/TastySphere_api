...
import com.example.tastysphere_api.dto.response.UserResponse;
...
public interface UserMapper {
    ...
    @Mapping(target = "avatar", expression = "java(formatAvatarUrl(user.getAvatar()))")
    ...
    default int getFollowersCount(User user) {
        // 实现获取关注者数量的逻辑
        return 0; // 实际实现中替换为真实逻辑
    }
    default int getFollowingCount(User user) {
        // 实现获取关注数量的逻辑
        return 0; // 实际实现中替换为真实逻辑
    }
    default String formatAvatarUrl(String avatar) {
        ...
        if (!avatar.startsWith("http://") && !avatar.startsWith("https://")) {
            ...
            return "https://your-domain.com/avatars/" + avatar;
        }
